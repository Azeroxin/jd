class Slider {
  ...
  animate(offset) {
    ...
    let animate = setInterval(() => {
      if (
        this.picBox.style.left == goal || Math.abs(Math.abs(parseFloat(this.picBox.style.left)) - Math.abs(goal)) < Math.abs(speed)
      ) {
        ...
        if (parseFloat(this.picBox.style.left) == 0) {
          ...
          parseFloat(this.picBox.style.left) == -(this.sliders + 1) * this.sliderWidth
        ) {
